...
class ImporterFile {
	...
	typeFickser(type) {
		switch (type) {
			...
			case "soundeffect":
			case "voiceover": {
				return { type: "sound", subtype: type, sound: true};
			} case "video": {
				return { type: "prop", subtype: type, sound: false };
			} default: {
				return { type: type, subtype: 0, sound: false };
			}
		}
	}
	upload(passedname) {
			...
			.done(d => {
				if (d.status == "ok") {
					if (IS_STUDIO) {
						...
						if (this.subtype == 0 || this.sound == true) {
							if (this.ext != "swf" && this.ext != "mp3" && this.ext != "wav" && this.ext != "ogg") 
